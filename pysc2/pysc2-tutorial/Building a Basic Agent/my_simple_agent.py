...
_BUILD_REF = actions.FUNCTIONS.Build_Refinery_screen.id
...
_REF_UNIT_TYPE = features.SCREEN_FEATURES.unit_density.index
...
class SimpleAgent(base_agent.BaseAgent):
    ...
    def step(self, obs):
        ...
        elif not self.ref_built:
            if _BUILD_REF in obs.observation["available_actions"]:
                unit_type = obs.observation["screen"][_REF_UNIT_TYPE]
                unit_y, unit_x = (unit_type == _VESPENE_GAS).nonzero()
                if unit_y.any():
                    target = [unit_x.mean(), unit_y.mean()]
                    self.ref_built = True
                    return actions.FunctionCall(_BUILD_REF, [_NOT_QUEUED, target])
                #if unit_y.any():
                    #target = [unit_x[0], unit_y[0]]
